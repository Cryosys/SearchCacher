...
namespace SearchCacher
{
    internal interface ISearcher
    {
        ...
        /// <summary>	Returns all statistics collected for the different root search paths </summary>
    ...
    internal class FileDBSearcher : ISearcher
    {
        ...
        public Statistics[] GetStatistics()
        {
            ...
            foreach (var config in _configs)
                if (config.Stats is not null)
                    stats.Add(config.Stats);
        ...
        private void _SaveDB(FileDBConfig config, bool parentHasMasterLock = false, bool force = false)
        {
            ...
            try
            {
                ...
                lock (config.SaveLockObj)
                {
                    ...
                    {
                        Program.Log("DB not dirty, skipping -> " + config.RootPath);
                        return;
                    }
                ...
                Program.Log("Saved DB -> " + config.RootPath);
            }
            catch (Exception ex)
            {
                CryLib.Core.LibTools.ExceptionManager.AddException(ex);
                Program.Log("Failed to save DB -> " + config.RootPath);
            }
            finally
            {
                if (!parentHasMasterLock)
        ...
        private class FileDBConfig
        {
            ...
            [JsonIgnore]
            internal Lock SaveLockObj { get; } = new();
